...
cat > tool/patch_brokers_add_missing.dart << 'DART'
import 'dart:io';
...
Future<void> _patchContinental(SupabaseClient client) async {
  ...
  final data = await client
      ...
      .or('name.ilike.$namePat,website.ilike.%$domain%,email.ilike.%@continental.mw%')
      .order('name')
      .limit(50);
  ...
  for (final r in rows) {
    ...
    final phoneFields = [currentPhone, currentAltPhone, patch['phone']?.toString() ?? ''];

    final newAlt1 = _appendDelimitedIfMissing(
      current: currentAltPhone,
      candidate: extraPhone1,
      alreadyHas: (cur, cand) => _hasNumberInFields(cand, [cur, ...phoneFields]),
    );
    ...
    if (patch.isEmpty) {
      print('Continental: ${r['name']} (id=$id) already has details; no changes.');
      continue;
    }
...
Future<void> _patchStockbrokersMw(SupabaseClient client) async {
  ...
  final data = await client
      .from('brokers')
      .select('id,name,address,phone,alt_phone,email,alt_email,website')
      .or('name.ilike.$namePat,website.ilike.%$domain%,email.ilike.%@smlmw.com%')
      .order('name')
      .limit(50);
  ...
  for (final r in rows) {
    ...
    var afterPrimary = _appendAddressLinesIfMissing(currentAddr, addrLinesPrimary) ?? currentAddr;
    ...
    if (patch.isEmpty) {
      print('StockbrokersMW: ${r['name']} (id=$id) already has details; no changes.');
      continue;
    }
    await client.from('brokers').update(patch).eq('id', id);
...
SUPABASE_URL="$SUPABASE_URL" SUPABASE_SERVICE_ROLE_KEY="$SUPABASE_SERVICE_ROLE_KEY" dart run tool/patch_brokers_add_missing.dart
