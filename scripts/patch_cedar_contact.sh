...
cat > tool/patch_cedar_contact.dart << 'DART'
import 'dart:io';
...
Future<void> main() async {
  ...
  for (final r in list) {
    ...
    if (patch.isEmpty) {
      print('OK: ${r['name']} (id=$id) already has these Cedar contact details.');
      continue;
    }
  ...
  print('Done. Added only missing Cedar Capital contact info; nothing existing was removed.');
}
DART

# Prompt secrets safely
read -rp "SUPABASE_URL: " SUPABASE_URL
read -rsp "SUPABASE_SERVICE_ROLE_KEY (hidden): " SUPABASE_SERVICE_ROLE_KEY
echo

SUPABASE_URL="$SUPABASE_URL" SUPABASE_SERVICE_ROLE_KEY="$SUPABASE_SERVICE_ROLE_KEY" dart run tool/patch_cedar_contact.dart
