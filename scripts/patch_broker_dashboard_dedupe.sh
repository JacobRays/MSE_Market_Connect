...
DST="lib/features/brokers/presentation/broker_dashboard_screen.dart"
...
class _BrokerDashboardScreenState extends State<BrokerDashboardScreen> {
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      if (_isAdmin) {
        final raw = await _brokerService.getActiveBrokers();
        final deduped = _dedupeBrokersByName(raw);
        if (raw.length != deduped.length) {
          // ignore: avoid_print
          print('BrokerDashboardScreen: removed ${raw.length - deduped.length} duplicate broker(s) by name');
        }
