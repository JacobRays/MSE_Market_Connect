...
DST="lib/features/brokers/presentation/broker_detail_screen.dart"
[[ -f "$DST" ]] || { echo "File not found: $DST" >&2; exit 1; }

ts="$(date +%Y%m%d_%H%M%S)"
cp -a "$DST" "${DST}.bak.${ts}"
echo "Backup saved: ${DST}.bak.${ts}"
...
class BrokerDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final whatsappDigits = (broker.whatsapp ?? '').replaceAll(RegExp(r'[^0-9]'), '').trim();
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      _Chip(
                        label: 'Fee ${(broker.feeRate * 100).toStringAsFixed(2)}%',
                      ),
            ...
            child: Column(
              children: [
                ...
                ListTile(
                  ...
                  subtitle: Text(whatsappDigits.isEmpty ? 'Not provided' : whatsappDigits),
                  trailing: const Icon(Icons.open_in_new),
                  onTap: waUri == null
                      ? null
                      : () => _open(
                  ...
                  onLongPress: whatsappDigits.isEmpty ? null : () => _copy(context, whatsappDigits, 'WhatsApp'),
                ),
                ...
                ListTile(
                  ...
                  subtitle: Text(websiteUri == null ? 'Not provided' : websiteUri.toString()),
                  trailing: const Icon(Icons.open_in_new),
          ...
          if (address.isNotEmpty) ...[
            ...
            Card(
              child: ListTile(
                ...
                title: Text(address),
                trailing: mapsUri == null ? null : const Icon(Icons.open_in_new),
                onTap: mapsUri == null
                    ? null
                    : () => _open(
                          context,
